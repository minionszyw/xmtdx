...
| 五档行情 | `get_security_quotes` | 已验证 | 单包上限 80，高层 API 自动分片；核对返回 market/code |
...
| 当日逐笔 | `get_transaction_data` | 已验证 | 保留 `num_orders` 与未知字段 |
| 历史逐笔 | `get_history_transaction_data` | 已验证 | `num_orders=None` |
...
| 资金流 | `get_fund_flow` / `get_history_fund_flow` | 受限 | 基于 L1 逐笔金额分档；Category 22 空时回退重算 |
